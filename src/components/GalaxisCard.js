...
const GalaxisCard = ({
  ...
  const GALAXIS_BASE_URL = apiUrl
    ? apiUrl
    : 'https://galaxis-backend.herokuapp.com';
  ...
  useEffect(() => {
    if (
      metadata.sides &&
      metadata.sides.length >= 1 &&
      metadata.sides[0].original_height &&
      metadata.sides[0].original_width
    ) {
      ...
      if (imageRatio != 0) {
        ...
        setResizerComponentSize({
          width:
            originalWidth < originalHeight / imageRatio
              ? 'auto'
              : originalHeight / imageRatio,
          height:
            originalWidth < originalHeight / imageRatio
              ? originalWidth * imageRatio
              : 'auto',
        });
      }
      setLoading(false);
            ...
            >
              ...
              >
                ...
                >
                  {(metadata.sides &&
                    metadata.sides.length >= 1 &&
                    (!metadata.sides[0].type ||
                      metadata.sides[0].type !== 'video')) ||
                  !metadata.sides ? (
                    <>
                      <img
                      ...
                      {metadata.overlays &&
                        ...
                        metadata.overlays[0].layers &&
                        metadata.overlays[0].layers[0].length > 0 && (
                          <img
                              ...
                              if (parseInt(elem.type) === traitElem.id) {
                                return (
                                  ...
                                  >
                                    ...
                                    <img
                                      src={
                                        GALAXIS_BASE_URL + traitElem.icon_white
                                      }
                                      alt="undefined"
                                    />{' '}
                                  </div>
                ...
                {metadata.sides && metadata.sides.length > 1 && (
                  ...
                  >
                    <CardBack
                      ...
                      type={
                        metadata.sides[1].type === 'video' &&
                        metadata.sides[1].type
                      }
                    />
                  </span>
                )}
